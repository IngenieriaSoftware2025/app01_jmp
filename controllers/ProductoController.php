...
class ProductoController {
    ...
    public static function marcarAPI() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $id = $_POST['prod_id'] ?? null;
            $valor = isset($_POST['valor']) ? (int)$_POST['valor'] : 0; // Convertir a entero

            if ($id !== null && ($valor === 0 || $valor === 1)) { // Validar que sea 0 o 1
                $producto = Productos::find($id);
                ...
                echo json_encode(['resultado' => false, 'mensaje' => 'Datos incorrectos']);
            }
        }
    }
}
